...
class VisitaController extends Controller
{
    ...
    {
        $reserva->load(['venta', 'programa.servicios', 'menus']);
        ...
        $masajesActuales = $reserva->masajes()->orderBy('persona')->get();
        $visitasActuales = $reserva->visitas()->orderBy('id')->get();
        ...
        return view('themes.backoffice.pages.visita.edit', [
            ...
            'masajesActuales'       => $masajesActuales,
            'menusActuales'         => $reserva->menus,
            'visitasActuales'       => $visitasActuales,
        ]);
    }
    ...
    public function actualizar(UpdateRequest $request, Reserva $reserva)
    {
        ...
        $masajesConHorario = $reserva->masajes->contains(function ($masaje) {
            return ! empty($masaje->horario_masaje);
        });
        ...
        try {
            DB::transaction(function () use (
                ...
                if ($tieneSpaReal) {
                    $visitaMail = $this->actualizarVisitasDesdeSpa($request, $reserva, $spas);
                } else {
                    $visitaMail = $visitasConHorario
                        ? $this->actualizarVisitasMetadatos($request, $reserva)
                        : $this->actualizarVisitasPlaceholder($request, $reserva);
                }
                ...
                if ($incluyeAlmuerzo) {
                    $this->actualizarMenusDesdePayload($request, $reserva);
                } else {
                    $reserva->menus()->delete();
                }
            ...
            return redirect()
                ->back()
                ->withInput()
                ->with('error', 'Ocurrió un error al actualizar la visita.');
        }
    }
    ...
    private function reservaIncluyeMasaje(Reserva $reserva): bool
    {
        if (! $reserva->relationLoaded('programa')) {
            $reserva->load('programa.servicios');
        }

        $servicios = optional(optional($reserva->programa)->servicios);

        if (! $servicios) {
            return false;
        }

        return $servicios->contains(function ($servicio) {
            $nombre = mb_strtolower(trim($servicio->nombre_servicio));
            return in_array($nombre, ['masaje', 'masajes']);
        });
    }
    ...
    private function actualizarVisitasDesdeSpa($request, Reserva $reserva, array $spas): ?Visita
    {
        $visitas = $reserva->visitas()->orderBy('id')->get();
        $indice  = 0;
        $ultima  = null;
        ...
        foreach ($spas as $spa) {
            ...
            $visita = $visitas[$indice] ?? null;

            if ($visita) {
                $visita->update($payload);
            } else {
                $visita = Visita::create($payload);
            }
        ...
        if (! $ultima) {
            return $this->actualizarVisitasPlaceholder($request, $reserva);
        }

        $totalVisitas = $visitas->count();
        for ($i = $indice; $i < $totalVisitas; $i++) {
            $visitas[$i]->update([
                'horario_sauna'  => null,
                'horario_tinaja' => null,
                'id_ubicacion'   => $request->input('id_ubicacion'),
    ...
    private function actualizarVisitasMetadatos($request, Reserva $reserva): ?Visita
    {
        $visitas = $reserva->visitas()->orderBy('id')->get();
        ...
        foreach ($visitas as $visita) {
            $visita->update([
                'id_ubicacion'   => $request->input('id_ubicacion', $visita->id_ubicacion),
                'trago_cortesia' => $request->input('trago_cortesia', $visita->trago_cortesia),
                'observacion'    => $request->input('observacion', $visita->observacion),
            ]);
    ...
    private function actualizarVisitasPlaceholder($request, Reserva $reserva): ?Visita
    {
        $cantidadPersonas     = (int) $reserva->cantidad_personas;
        $maxPersonasPorVisita = 5;
        $necesarias           = (int) ceil($cantidadPersonas / $maxPersonasPorVisita);

        $visitas = $reserva->visitas()->orderBy('id')->get();

        if ($visitas->count() < $necesarias) {
            $faltantes = $necesarias - $visitas->count();
    ...
    private function asegurarMasajesParaPersonas(Reserva $reserva, int $personasMasaje, bool $resetValores = false, bool $incluyeMasajePrograma = false)
    {
        ...
        for ($persona = 1; $persona <= $personasMasaje; $persona++) {
        ...
        Masaje::where('id_reserva', $reserva->id)
            ->where('persona', '>', $personasMasaje)
            ->delete();

        return $reserva->masajes()->orderBy('persona')->get();
    }

    private function actualizarMasajesDesdePayload(Reserva $reserva, array $masajesNorm, int $personasMasaje): void
    {
        $modo = collect($masajesNorm)->contains(function ($m) {return $m['modo'] === 'extra';}) ? 'extra' : 'incluido';
        ...
        if ($modo === 'extra') {
            ...
            foreach ($masajesNorm as $m) {
                ...
                if ($indicePersona >= $personasMasaje) {
                    break;
                }
                ...
                $masajes[$indicePersona]->update([
                    'horario_masaje'  => Carbon::createFromFormat('H:i', $m['horario_masaje']),
                    'tipo_masaje'     => $m['tipo_slug'] ? ($mapTipoNombre[$m['tipo_slug']] ?? null) : null,
                    'id_lugar_masaje' => (int) ($m['id_lugar_masaje'] ?? 1),
                    'tiempo_extra'    => ($duracion !== 30 && $duracion > 0) ? true : false,
                ]);
        ...
        foreach ($masajesNorm as $slot) {
            ...
            $horario = Carbon::createFromFormat('H:i', $slot['horario_masaje']);

            for ($i = 0; $i < $maxPersonasHorario; $i++) {
                if ($indicePersona >= $personasMasaje) {
                    break 2;
                }
    ...
    private function actualizarMenusDesdePayload($request, Reserva $reserva): void
    {
        $cantidadNecesaria = max(0, (int) $reserva->cantidad_personas);
        $menus             = $this->asegurarMenusParaReserva($reserva, $cantidadNecesaria);
        $payload           = array_values($request->input('menus', []));

        foreach ($menus as $index => $menu) {
            $data = $payload[$index] ?? [];

            $menu->update([
                'id_producto_entrada'        => $data['id_producto_entrada'] ?? null,
                'id_producto_fondo'          => $data['id_producto_fondo'] ?? null,
                'id_producto_acompanamiento' => $data['id_producto_acompanamiento'] ?? null,
    ...
    private function asegurarMenusParaReserva(Reserva $reserva, int $cantidadNecesaria)
    {
        $menus     = $reserva->menus()->orderBy('id')->get();
        $faltantes = $cantidadNecesaria - $menus->count();
        ...
        return $reserva->menus()->orderBy('id')->get();
    }
