...
@section('title','Editar Venta Poro Poro')
...
@section('content')
<div class="section">
    <p class="caption">Modifique los datos de la venta registrada.</p>
    <div class="divider"></div>
    <div class="row">
        <div class="col s12 m10 offset-m1">
            <div class="card-panel">
                <h4 class="header">Editar venta <strong>Poro Poro</strong></h4>

                <form method="POST" action="{{ route('backoffice.ventas_poroporo.update', $poroVenta->id) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="input-field col s12">
                            <select name="id_tipo_transaccion">
                                ...
                                @foreach ($tiposTransacciones as $tipo)
                                    <option value="{{ $tipo->id }}" {{ $poroVenta->id_tipo_transaccion == $tipo->id ? 'selected' : '' }}>
                                        {{ $tipo->nombre }}
                                    </option>
                                @endforeach
                            </select>
...
<script>
    const productosIniciales = @json($productosIniciales);
</script>
...
document.addEventListener('DOMContentLoaded', function () {
    if (!window._yaCargueListenerProductos) {
        ...
        // Precargar productos de la venta
        productosIniciales.forEach(detalle => {
            const id = detalle.id;
            const nombre = detalle.nombre;
            const valor = detalle.valor;
            const cantidad = detalle.cantidad;

            productosAgregados[id] = { cantidad, valor };

            const row = document.createElement('tr');
            row.setAttribute('data-id', id);
            row.innerHTML = `
                <td>${nombre}</td>
                <td><input type="number" min="1" value="${cantidad}" class="cantidad-input" style="width:60px;"></td>
                <td class="subtotal">$${(valor * cantidad).toLocaleString()}</td>
            ...
            if (productoOriginal) productoOriginal.style.display = 'none';
            ...
            row.querySelector('.eliminar-producto').addEventListener('click', function () {
                ...
                const productoOriginal = document.querySelector(`.producto-item[data-id="${id}"]`);
                if (productoOriginal) productoOriginal.style.display = 'block';
            });
        });
        ...
        lista.addEventListener('click', function (e) {
            ...
            if (productosAgregados[id]) return;
