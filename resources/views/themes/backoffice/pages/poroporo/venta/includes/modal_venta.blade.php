...
<div id="modal{{$poroVenta->id}}" class="modal modal-fixed-footer">
    ...
    <div class="modal-content">
        <div class="boleta-contenido">
            ...
            <div class="row">
                <div class="col s12">
                    <p class="col s6 left"><strong>Generada por:</strong> <span>{{ $poroVenta->user->name }}</span></p>
                    <p class="col s6 right"><strong>Fecha:</strong> <span>{{  \Carbon\Carbon::parse($poroVenta->fecha)->format('d-m-Y')  }}</span></p>
                </div>
            </div>
...
<div class="row">
    <div class="col s12">
      <div class="col s12">
        ...
        @if (isset($poroVenta->detalles))
          @foreach ($poroVenta->detalles as $detalle)
              @php 
              ...
              <div class="row">
                <div class="col s8 offset-s2">
                  <div class="col s6 left">{{ $detalle->poro->nombre }}</div>
...
<p><strong>Total a pagar:</strong> ${{ number_format($poroVenta->total, 0, ',', '.') }}</p>
    ...
    <div class="modal-footer">
      <button class="waves-effect waves-light btn" onclick="enviarFormulario({{$poroVenta->id}});">Imprimir</button>

      <a href="#!" class="modal-action modal-close waves-effect waves-red btn-flat">Cerrar</a>
    </div>


    <form id="boleta-form-{{$poroVenta->id}}" action="{{ route('backoffice.boleta.poro_poro',['poroVenta'=>$poroVenta]) }}" method="POST" style="display: none;">
      @csrf
    </form>
