<div class="col s12 hide-on-small-only hide-on-med-only">
    <h5>Reservas: {{ \Carbon\Carbon::parse($fecha)->format('d-m-Y') }}</h5>
    <div class="row">
        {{-- Sauna --}}
                    ...
                    <tbody>
                        @php
                            ...
                            foreach ($reservas as $r) {
                                ...
                                // Color por reserva (misma lógica tuya)
                                ...
                                foreach ($r->visitas as $v) {
                                    if (!$v->horario_sauna) continue;

                                    $keyHora = $v->horario_sauna;
                                    if (!isset($slotsSauna[$keyHora])) $slotsSauna[$keyHora] = [];

                                    $slotsSauna[$keyHora][] = [
                                        'inicio'      => $v->horario_sauna,
                                        'fin'         => $v->hora_fin_sauna,
                                        'cliente'     => $r->cliente->nombre_cliente,
                                        'ubicacion'   => optional($v->ubicacion)->nombre ?? 'No registra',
                                        'programa'    => $r->programa->nombre_programa,
                                        'personas'    => $r->cantidad_personas,
                                        'observacion' => $r->observacion,
                                        'trago'       => $v->trago_cortesia,
                                        'color'       => $color,
                            ...
                            // Orden global por hora
                            ksort($slotsSauna);
                        @endphp

                        @foreach($slotsSauna as $hora => $items)
                    ...
                    <tbody>
                        @php
                            ...
                            foreach ($reservas as $r) {
                                ...
                                foreach ($r->visitas as $v) {
                                    if (!$v->horario_tinaja) continue;

                                    $keyHora = $v->horario_tinaja;
                                    if (!isset($slotsTinaja[$keyHora])) $slotsTinaja[$keyHora] = [];

                                    $slotsTinaja[$keyHora][] = [
                                        'inicio'      => $v->horario_tinaja,
                                        'fin'         => $v->hora_fin_tinaja,
                                        'cliente'     => $r->cliente->nombre_cliente,
                                        'ubicacion'   => optional($v->ubicacion)->nombre ?? 'No registra',
                                        'programa'    => $r->programa->nombre_programa,
                                        'personas'    => $r->cantidad_personas,
                        ...
                        @foreach($slotsTinaja as $hora => $items)
                            @foreach($items as $row)
                                <tr>
                                    <td class="{{ $row['color'] }}" style="border-radius:5px">
                                        <strong style="color:#F5F5F5; display:flex; justify-content:center; flex-direction:column">
